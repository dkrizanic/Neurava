...
**Approach:** Publish note content change events from the notes module and let a separate memory module maintain PostgreSQL/pgvector retrieval records containing safe searchable text, workspace metadata, and embeddings.

## Boundaries & Constraints

**Always:** Retrieval records are scoped to workspace context id; notes must not call memory services directly; indexing failures must be logged and must not roll back note saves; stored text must be title/body metadata only.
...
**Never:** Do not expose embeddings to the frontend, leak OAuth/provider payloads, or import note persistence entities into the memory module.
...
- `apps/api/src/main/java/com/notebook/api/memory/**` -- memory module listener, embedding generator, and JDBC index repository.
- `apps/api/src/test/java/com/notebook/api/memory/**` -- indexing service and listener coverage.
...
- [x] `apps/api/src/main/java/com/notebook/api/notes/**` -- publish note content events -- decouples notes from memory indexing.
- [x] `apps/api/src/main/java/com/notebook/api/memory/**` -- index note content from events -- prepares searchable text and embedding vectors.
...
Implemented. Notes now publish content-change events after create/update, and the memory module handles those events after commit to maintain a workspace-scoped pgvector retrieval index. The implementation keeps notes independent from memory, stores only title/body searchable text plus metadata, and logs indexing failures without rolling back note saves.
